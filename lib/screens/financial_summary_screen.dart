...
class _FinancialSummaryScreenState extends State<FinancialSummaryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    String highestCategory = categoryTotals.isNotEmpty
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSummaryCard("Total Expenses", totalExpenses, Colors.red),
            _buildSummaryCard("Balance", balance, Colors.blue),
            _buildSummaryCard("Savings Rate", savingsRate, Colors.purple, isPercentage: true),
            // _buildSummaryCard("Top Spending Category", highestCategory, Colors.orange, isAmount: false),
            const SizedBox(height: 20),
            const Text("Expense Breakdown", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            categoryTotals.isEmpty
                    ...
                    child: PieChart(
                      PieChartData(
                        sections: categoryTotals.entries.map((entry) {
                          return PieChartSectionData(
                            value: entry.value,
                            title: "${entry.key}\n\Frw ${entry.value.toStringAsFixed(0)}",
                            color: _getCategoryColor(entry.key),
                            radius: 80,
                            titleStyle: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold),
                          );
  ...
  Widget _buildSummaryCard(String title, dynamic value, Color color, {bool isPercentage = false, bool isAmount = true}) {
    String displayValue;
    if (!isAmount) {
      ...
      displayValue = "${value.toStringAsFixed(2)} Frw";
    }
    return Card(
      elevation: 3,
      margin: const EdgeInsets.symmetric(vertical: 8),
      child: ListTile(
        title: Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        trailing: Text(displayValue, style: TextStyle(fontSize: 18, color: color)),
      ),
    );
  }
